...
<body>
     ...
     * - Replaces wammy-novel-image:// src attributes with relative EPUB image paths.
     * - Removes orphan wammy-novel-image:// img elements (image not in [images]).
    ...
    private fun rewriteChapterHtml(content: String, chapterPrefix: String, images: List<EmbeddedImage>): String {
        val hasWammyPaths = content.contains("wammy-novel-image://")
        val hasPictures = content.contains("<picture")
        if (images.isEmpty() && !hasWammyPaths && !hasPictures) return content
        ...
        // Replace / remove wammy-novel-image:// src attributes
        doc.select("img[src]").forEach { imgEl ->
            val src = imgEl.attr("src")
            if (src.startsWith("wammy-novel-image://")) {
...
$navPoints
    ...
    private fun writePackageDocument(
        ...
        val manifestItems = buildString {
            ...
            if (hasCustomCss) {
                appendLine("""        <item id="wammy-style" href="$CUSTOM_CSS_PATH" media-type="text/css"/>""")
            }
            if (hasCustomJs) {
                appendLine(
                    """        <item id="wammy-script" href="$CUSTOM_JS_PATH" media-type="application/javascript"/>""",
                )
            }
            chapters.forEachIndexed { chIdx, chapter ->
...
$spineItems
    ...
    companion object {
        ...
        const val CUSTOM_CSS_PATH = "styles/wammy-style.css"
        const val CUSTOM_JS_PATH = "scripts/wammy-script.js"
    }
}
