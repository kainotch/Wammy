...
internal class CoilImageGetter(
    ...
    override fun getDrawable(source: String?): Drawable {
        ...
        if (source.startsWith("wammy-novel-image://") ||
    ...
    fun startLoading() {
        ...
        for ((source, wrapper) in pendingLoads) {
            when {
                source.startsWith("wammy-novel-image://") -> loadFromPageLoader(source, wrapper, loader)
    ...
    private fun loadFromPageLoader(source: String, wrapper: DrawableWrapper, loader: PageLoader?) {
        scope.launch(Dispatchers.IO) {
            try {
                val imagePath = android.net.Uri.decode(source.removePrefix("wammy-novel-image://"))
