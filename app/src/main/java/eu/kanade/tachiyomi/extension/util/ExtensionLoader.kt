...
internal object ExtensionLoader {
    ...
    private suspend fun loadExtension(context: Context, extensionInfo: ExtensionInfo): LoadResult {
        ...
        // Prefer tachiyomix.name metadata; fall back to app label with old "Tachiyomi: " or "Wammy: " prefixes
        val extName = appInfo.metaData.getString(METADATA_NAME)
            ?: pkgManager.getApplicationLabel(appInfo).toString().let { appLabel ->
                when {
                    appLabel.startsWith("Wammy: ") -> appLabel.substringAfter("Wammy: ")
        ...
        val sources = (
            ...
            .flatMap { className ->
                ...
                // If the package uses com.notch.wammy namespace, also try eu.kanade.tachiyomi namespace
                // This handles extensions that were migrated to new package names but still have
                // classes in the old namespace
                if (className.startsWith("com.notch.wammy.extension.")) {
                    val fallbackClass = className.replace("com.notch.wammy.extension.", "eu.kanade.tachiyomi.extension.")
