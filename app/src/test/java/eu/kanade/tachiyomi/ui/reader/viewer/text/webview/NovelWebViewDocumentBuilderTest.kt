...
class NovelWebViewDocumentBuilderTest {
    ...
    // â”€â”€ escapeForStyleTag â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ extractBodyOrFallback â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ assemble: structural checks â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fun `assemble escapes malicious closing style in user css`() {
        ...
        val styleTagContent = html.substringAfter("wammy-custom-style\">").substringBefore("</style>")
        assertFalse(styleTagContent.contains("</style>"))
    }

    @Test
    fun `assemble hides chapter headings with wammy-chapter scoped selector`() {
        val html = NovelWebViewDocumentBuilder.assemble(minimalInput(hideChapterTitle = true))
        // Selector must be scoped to wammy-chapter, not document-wide
        assertTrue(html.contains("wammy-chapter h1:first-of-type"))
    ...
    fun `assemble does not produce the divider's visible-boundary rule without infinite scroll`() {
        ...
        // paged-mode override (which forces the divider invisible under html.wammy-paged
    ...
    // Paged mode CSS: always embedded, gated at runtime by the .wammy-paged class

    @Test
    fun `assemble always embeds the paged-mode clip and column CSS`() {
        val html = NovelWebViewDocumentBuilder.assemble(minimalInput())
        assertTrue(html.contains("html.wammy-paged"))
        assertTrue(html.contains("html.wammy-paged body"))
        assertTrue(html.contains("html.wammy-paged #${NovelWebViewChapterMeta.WAMMY_CHAPTERS_CONTAINER_ID}"))
    }
    ...
    fun `assemble avoids splitting media elements across a page break`() {
        ...
        val styleSection = html.substringAfter("<style>").substringBefore("</style>")
        val breakRule = Regex(
            "html\\.wammy-paged[^{]*\\{[^}]*break-inside:\\s*avoid",
            RegexOption.DOT_MATCHES_ALL,
        )
        assertTrue(breakRule.containsMatchIn(styleSection)) {
            "Expected a break-inside: avoid rule scoped to .wammy-paged, got: $styleSection"
        }
        for (tag in listOf("img", "table", "pre", "blockquote", "figure")) {
            assertTrue("html.wammy-paged $tag" in styleSection) { "Missing paged break-inside rule for <$tag>" }
        }
    }
    ...
    fun `assemble forces the chapter divider invisible under paged mode regardless of infinite scroll`() {
        ...
        assertTrue(
            html.contains(
                "html.wammy-paged .${NovelWebViewChapterMeta.CHAPTER_DIVIDER_CLASS}",
            ),
        )
    }
}
